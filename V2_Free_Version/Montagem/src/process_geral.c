...
// > RESET MARCAÇÃO
LL_antena ResetMarc(LL_antena inicioA){
    LL_antena aux = inicioA;
    while(aux){
        aux->marc=0;
        aux=aux->seguinte;
    }
...
LL_antena MarcaAntena(LL_antena inicioA, int linha, int coluna){
    LL_antena aux = inicioA;
    while(aux){
        if((aux->m == linha) && (aux->n == coluna)){
            aux->marc=aux->marc +1;
            break;
        }
        aux=aux->seguinte;
...
// > MANIPULAÇÃO DA LISTA DE ANTENAS

/**
 * @brief Insere uma nova antena na lista ligada ordenada por linha e coluna.
 * 
 * Esta função verifica se já existe uma antena na posição (linha, coluna). Se não existir,
 * ela insere a antena na posição correta da lista, mantendo a ordem crescente de linha e coluna.
 *
 * @param inicio Ponteiro para o início da lista ligada de antenas.
 * @param linha Número da linha da antena a ser inserida.
 * @param coluna Número da coluna da antena a ser inserida.
 * @param frequencia Frequência da antena a ser inserida.
 * 
 * @return Ponteiro para o início da lista ligada (pode ser modificado caso a antena seja inserida no início).
 */
LL_antena InserirAntena(LL_antena inicio, int linha, int coluna, char frequencia) {
    // Alocar memória para a nova antena
    LL_antena nova_antena = malloc(sizeof(struct antena));
    if (nova_antena == NULL) {
        FalhaMalloc(inicio, NULL); 
    }

    // Preencher dados da antena
    nova_antena->m = linha;
    nova_antena->n = coluna;
    nova_antena->f = frequencia;
    nova_antena->marc=0;
    nova_antena->seguinte = NULL;

    // Se a lista estiver vazia
    if (inicio == NULL) {
        printf("\n> Elemento (%d, %d) adicionada à lista.\n", linha, coluna);
        return nova_antena;
    }

    LL_antena actual = inicio, anterior = NULL;

    // Percorrer a lista para verificar duplicados ou encontrar posição
    while (actual != NULL) {
        if (actual->m == linha && actual->n == coluna) {
            printf("\n\33[31m> Já existe uma antena nessa localização.\33[0m\n");
            free(nova_antena);
            return inicio;
        }

        if (actual->m > linha || (actual->m == linha && actual->n > coluna)) {
            break;
        }
    ...
    if (anterior == NULL) {
        nova_antena->seguinte = inicio;
        printf("\n> Elemento (%d, %d) adicionado com sucesso.\n", linha, coluna);
        return nova_antena;
    }

    // Inserir no meio ou fim da lista
    nova_antena->seguinte = actual;
    anterior->seguinte = nova_antena;
...
/**
 * @brief Remove uma antena da lista ligada com base na posição (linha, coluna).
 * 
 * Esta função percorre a lista e, se encontrar a antena na posição especificada, remove-a.
 * Caso não encontre a antena, exibe uma mensagem de erro.
 *
 * @param inicio Ponteiro para o início da lista ligada de antenas.
 * @param linha Número da linha da antena a ser removida.
 * @param coluna Número da coluna da antena a ser removida.
 * 
 * @return Ponteiro para o início da lista ligada (pode ser modificado caso a antena seja removida).
 */
LL_antena RemoverAntena(LL_antena inicio, int linha, int coluna) {
    ...
    if (inicio == NULL) 
    {
        printf("\n\33[31m> Atualmente sem Elementos.\33[0m\n");
        return inicio;
    }

    LL_antena actual = inicio, anterior = NULL;

    // Percorrer a lista à procura da antena a remover
    while (actual != NULL) {
        // Verificar se a antena tem a posição (linha, coluna) que corresponde
        if (actual->m == linha && actual->n == coluna) {
            // Se for o primeiro elemento da lista, mover o início para o próximo
            if (anterior == NULL) inicio = actual->seguinte;
            else anterior->seguinte = actual->seguinte;

            // Liberar a memória da antena removida
            printf("\n\n> Elemento (%d, %d) removido com sucesso\n", actual->m, actual->n);
            free(actual);
            return inicio;
        }

        // Continuar a procurar na lista
        anterior = actual;
        actual = actual->seguinte;
    }

    // Se não encontrar a antena, exibe uma mensagem de erro
    printf("\n\33[31m> Não existe nenhum elemento nesta localização\n\33[0m");
    return inicio;
}
