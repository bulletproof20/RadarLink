...
void intro (){
    ...
    printf("==============================================================\n");
    printf("-> Lê matriz de antenas a partir de ficheiro de texto\n");
    printf("-> A frequência de cada antena é representada por um carácter\n");
    printf("-> Apenas existe ligação entre antenas com a mesma frequência\n");
    printf("-> Permite procura de antenas ligadas (DFS e BFS)\n");
    printf("-> Permite analisar caminhos possíveis entre duas antenas\n");
    printf("==============================================================");
}
...
void menu(){
    ...
    printf("   > \33[32m(1)\33[0mProcura em profundidade a partir de uma antena.\n");
    printf("   > \33[32m(2)\33[0mProcura em largura a partir de uma antena.\n");
    printf("   > \33[32m(3)\33[0mIdentificar todos caminhos possíveis entre duas antenas.\n");
    printf("   > \33[32m(4)\33[0mVerificar cruzamento entre 2 arestas de frequencias distintas.\n");
...
LL_antena ApresentaMatriz(LL_antena inicio, int n_linhas, int n_colunas) {
    ...
    for (int linha = 0; linha < n_linhas; linha++) {
        ...
        for (int coluna = 0; coluna < n_colunas; coluna++) {
            ...
            {
                // Imprime o caractere da antena (frequência)s
                printf("%c", aux->f);
...
void MostrarIntersecoes(LL_Intercessoes inicioI, LL_segmento inicioSg, LL_subgrafos inicioS) {
    ...
    while (inter) { ///< Itera sobre cada interseção da lista
        for (int i = 0; i < 2; i++) { ///< Itera duas vezes: uma para cada segmento da interseção
            ...
            if (i == 0) {
                printf("\n\n> Segmento %d, f: '%c' ((%d, %d)-(%d, %d))", 
                       inter->s1, seg->freq, vert1->m, vert1->n, vert2->m, vert2->n);
                printf("\n--- Interceta ---\n");
            } else {
                printf("> Segmento %d, f: '%c' ((%d, %d)-(%d, %d))", 
...
int ObterVertice(LL_subgrafos inicioS, char frequencia) {
    ...
    while (aux == NULL) {
        ...
        if (!aux) {
            ...
            printf("\n> Selecione um ID de vértice válido.\n");
        }
    }
